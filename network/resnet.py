...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.downsample = downsample # 用来处理resnet结构相加 唯独不同的情况
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer1 = self._make_layer(block, 64, layers[0]) # 上一个已经maxpool了 因此保持 56 56
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False): # planes 基准通道数 64 128 256 512
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer)) # 减小尺寸
    ...
    def _forward_impl(self, x):
        ...
        x = self.conv1(x) # static structure
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        x = self.avgpool(x)
        # print(x.shape)
        avg = torch.flatten(x, 1) # 2048
        x = self.fc(avg) # 1000

        return x.squeeze(), avg.squeeze(), f_map.squeeze()
...
def get_resnet_model(resnet_type=152):
    ...
    if resnet_type == 18:
        ...
        return resnet152(pretrained=True, progress=True)
